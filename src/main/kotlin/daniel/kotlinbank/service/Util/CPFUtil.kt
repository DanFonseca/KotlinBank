package daniel.kotlinbank.service.Util

import daniel.kotlinbank.errors.CPFInvalidException

class CPFUtil{
    companion object {
        fun myValidateCPF(cpf : String) {
            val cpfClean = cpf.replace(".", "").replace("-", "")

            //## check if size is eleven
            if (cpfClean.length != 11)
                throw CPFInvalidException("CPF Must Has 11 Characters")

            //## check if is number
            try {
                val number  = cpfClean.toLong()
            }catch (e : NumberFormatException){
                throw NumberFormatException("CPF Must Be a Number")
            }
            ...
            if (dvForTenthDigit != dvCurrent10)
                throw CPFInvalidException("CPF is invalid")
